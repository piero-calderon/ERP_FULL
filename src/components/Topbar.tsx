...
const ROL_LABELS_IT: Record<string, string> = {
  admin: "Super Admin",
  comercial: "Area Commerciale",
  operativa: "Area Operativa",
  empresarial: "Area Aziendale",
...
export function Topbar() {
  ...
  const rolLabels = lang === "es" ? ROL_LABELS_ES : ROL_LABELS_IT;
      ...
      <div className="flex items-center gap-2 lg:gap-6">
        ...
        <div className="flex items-center gap-1 lg:gap-3">
          ...
          <button onClick={toggleLang}
            className="h-10 px-3 flex items-center justify-center rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 transition-all active:scale-95 font-bold text-xs gap-1.5 border border-slate-200 dark:border-slate-700"
            title={lang === "es" ? "Cambia a italiano" : "Cambia a spagnolo"}>
            <span className={lang === "es" ? "text-blue-600 font-black" : "opacity-40"}>ES</span>
            <span className="text-slate-300 dark:text-slate-600">/</span>
            <span className={lang === "it" ? "text-blue-600 font-black" : "opacity-40"}>IT</span>
          </button>
          ...
          <button onClick={toggleTheme}
            ...
            title={theme === "light" ? t.modoOscuro : t.modoClaro}>
        ...
        <div className="flex items-center gap-3 p-1.5 rounded-xl">
          <div className="text-right hidden sm:block">
            ...
            <p className="text-[10px] text-blue-600 mt-1 font-bold uppercase tracking-wider">{rolLabels[usuario?.rol ?? "admin"] ?? "Super Admin"}</p>
          </div>
