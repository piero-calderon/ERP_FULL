// Modulo 6 - Logistica TMS - Modal base reutilizable
...
export const TmsModal: React.FC<TmsModalProps> = ({ titulo, subtitulo, onCerrar, onGuardar, guardando, children, size = "md" }) => {
...
export const Campo: React.FC<{ label: string; required?: boolean; error?: string; children: React.ReactNode }> = ({ label, required, error, children }) => (
...
export const Input: React.FC<InputProps> = ({ error, className, ...props }) => (
  <input className={cn("w-full px-3 py-2 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors", error ? "border-red-300 bg-red-50" : "border-slate-200 hover:border-slate-300", className)} {...props} />
);

interface SelectProps extends React.SelectHTMLAttributes<HTMLSelectElement> { error?: boolean; options: { value: string; label: string }[]; }
...
export const Textarea: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement> & { error?: boolean }> = ({ error, className, ...props }) => (
...
export const Seccion: React.FC<{ titulo: string }> = ({ titulo }) => (
  <div className="pt-2"><p className="text-xs font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 pb-2">{titulo}</p></div>
);
